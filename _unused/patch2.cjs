...
const regex = /try\s*\{\s*log\('STEP 1', 'Starting Image Upload'\);[\s\S]*?(?=\/\/ COMPLETION)/;
...
const newContent = content.replace(regex, updatedLogic);
fs.writeFileSync(file, newContent, 'utf8');
console.log('Successfully patched ebay_lister.js!');
