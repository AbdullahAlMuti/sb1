...
const endLine = 840; // index 840 is line 841 (the line before // 🔍 Page Readiness Check)
...
const updatedLogicLines = `async function runEbayAutomation(data) {
  ...
  async function waitFor(selectorFnOrSelector, timeoutMs = 8000, intervalMs = 200) {
    const startTime = Date.now();
    while (Date.now() - startTime < timeoutMs) {
      let el = typeof selectorFnOrSelector === 'function' ? selectorFnOrSelector() : document.querySelector(selectorFnOrSelector);
      if (el && el.offsetParent !== null) return el;
      await wait(intervalMs);
    }
    throw new Error('TIMEOUT: not found after ' + timeoutMs + 'ms');
  }
  ...
  function findByLabel(labelText) {
    const selectorLabel = labelText.replace(/"/g, '\\"');
    let el = document.querySelector(\`[aria-label*="\${selectorLabel}" i]\`);
    if (el) return el;
    const labels = Array.from(document.querySelectorAll('label'));
    const targetLabel = labels.find(l => l.textContent && l.textContent.trim().toLowerCase().includes(labelText.toLowerCase()));
    if (targetLabel) {
      if (targetLabel.hasAttribute('for')) { el = document.getElementById(targetLabel.getAttribute('for')); if (el) return el; }
      el = targetLabel.nextElementSibling;
      if (el && el.tagName.toLowerCase() === 'input') return el;
    }
    el = document.querySelector(\`[placeholder*="\${selectorLabel}" i]\`);
    if (el) return el;
    return null;
  }
  async function clickAndWait(element, waitMs = 400) {
  ...
  try {
    ...
    if (auctionData.title) {
        ...
        try {
            titleElement = await waitFor(() => findByLabel('title') || document.querySelector('[aria-label*="title" i]') || document.querySelector('[placeholder*="title" i]') || document.querySelector('[name*="title" i]') || document.querySelector('[data-testid*="title" i]'), 8000);
        } catch(e) {}
        if (!titleElement) { log('STEP 2', 'ERROR: Title field not found. Skipping.'); }
        else {
    ...
    if (auctionData.sku) {
        let skuElement = null;
        try {
            skuElement = await waitFor(() => findByLabel('custom label') || findByLabel('sku') || document.querySelector('[aria-label*="custom label" i]') || document.querySelector('[aria-label*="sku" i]') || document.querySelector('[placeholder*="custom label" i]') || document.querySelector('[placeholder*="sku" i]'), 8000);
        } catch(e) {}
    ...
    if (!auctionData.specs || !Array.isArray(auctionData.specs) || auctionData.specs.length === 0) log('STEP 5', 'SKIP: No custom specs provided. Continuing.');
      ...
      try { addCustomBtn = await waitFor(() => Array.from(document.querySelectorAll('button, a')).find(el => el.textContent && el.textContent.trim().toLowerCase().includes('add custom item specific')), 5000); } catch(e) {}
      if (!addCustomBtn) log('STEP 5', 'SKIP: Add custom item specific button not found. Continuing.');
        ...
        for (const spec of auctionData.specs) {
          ...
          let modal;
          try { modal = await waitFor('[role="dialog"], .modal, [data-testid*="modal"]', 5000); } catch (e) { log('STEP 5', 'ERROR: Modal did not open for spec: ' + spec.name + '. Skipping this spec.'); continue; }
    ...
    try { conditionField = await waitFor(() => findByLabel('condition') || document.querySelector('[aria-label*="condition" i]') || document.querySelector('[data-testid*="condition" i]'), 8000); } catch(e) {}
    ...
    if (auctionData.description) {
        ...
        let iframeDesc = null;
        try { iframeDesc = await waitFor(() => document.querySelector('iframe[id*="desc"], iframe[title*="description" i], iframe[aria-label*="description" i]'), 8000); } catch(e) {}
    ...
    if (auctionData.price) {
        let priceField = null;
        try { priceField = await waitFor(() => findByLabel('price') || document.querySelector('[aria-label*="price" i]') || document.querySelector('[data-testid*="price" i]') || document.querySelector('[placeholder*="price" i]') || document.querySelector('input[name*="price" i]'), 8000); } catch(e) {}
    ...
    try { countryField = await waitFor(() => findByLabel('country of origin') || document.querySelector('[aria-label*="country of origin" i]') || document.querySelector('[data-testid*="country" i]'), 5000); } catch(e) {}
...
console.log('Successfully patched runEbayAutomation correctly!');
