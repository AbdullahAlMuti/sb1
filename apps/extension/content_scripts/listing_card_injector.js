...
(function () {
    ...
    aliexpress: {
      ...
      matchListingPage() {
        ...
        return (
          p === '/' ||
          p === '/index.html' ||
          /^\/w\/wholesale-/i.test(p) ||
          /^\/wholesale/i.test(p) ||
          /^\/category\//i.test(p) ||
          location.search.includes('SearchText=') ||
          location.search.includes('searchText=') ||
          location.search.includes('trafficChannel=main') ||
      ...
      matchDetailPage() {
        return (
          /\/item\/\d+(?:\.html)?/i.test(location.pathname) ||
          !!document.querySelector('[data-pl="product-title"], h1')
        );
      },
      ...
      findContainers() {
        ...
        function uniqueProductLinkCount(el) {
          ...
          return new Set(
            Array.from(el.querySelectorAll('a[href*="/item/"]'))
        ...
        function hasProductText(el) {
          ...
          const hasTitleLink = Array.from(el.querySelectorAll('a[href*="/item/"]'))
            .some(link => safeText(link).replace(/\s+/g, ' ').trim().length > 12);
        ...
        function findProductCard(node) {
          ...
          for (let depth = 0; current && depth < 8; depth += 1, current = current.parentElement) {
            ...
            const productLinks = uniqueProductLinkCount(current);
            if (productLinks > 4) break;
            if (current.matches?.('[data-product-id], [data-item-id]') || hasProductText(current)) {
              best = current;
            }
          }
          return best ||
            node.closest?.('[data-product-id], [data-item-id], [class*="product"], [class*="item"], li, div') ||
            node;
        }
        ...
        selectors.forEach((selector) => {
          document.querySelectorAll(selector).forEach((node) => {
            ...
            if (
              card &&
              card.querySelector &&
              (card.matches?.('a[href*="/item/"]') || card.querySelector('a[href*="/item/"]'))
            ) {
              cards.add(card);
            }
          });
      ...
      readProductLinkTitle(link) {
        ...
        return candidates
          ...
          .find(value => this.isMeaningfulProductTitle(value)) || '';
      },

      findProductTitleLink(el) {
        const links = Array.from(el.querySelectorAll?.('a[href*="/item/"]') || []);
        return links.find(link => this.readProductLinkTitle(link)) || null;
      },
        ...
        if (rawHref) {
          try {
            ...
            url = rawHref.startsWith('http')
              ? rawHref.split('?')[0]
              : 'https://www.aliexpress.com' + rawHref.split('?')[0];
          }
        }
        const productId = (/\/item\/(\d+)(?:\.html)?/i.exec(url) || [])[1] || el.dataset.productId || el.dataset.itemId || '';
        const titleEl =
          this.findProductTitleLink(el) ||
          el.querySelector('[class*="title"] a[href*="/item/"]') ||
          el.querySelector('[class*="title"]') ||
          el.querySelector('h1, h2, h3') ||
          linkEl;
        const title =
          this.readProductLinkTitle(titleEl) ||
          (this.isMeaningfulProductTitle(safeText(titleEl)) ? safeText(titleEl) : '') ||
          (this.isMeaningfulProductTitle(titleEl?.getAttribute?.('title')) ? titleEl.getAttribute('title') : '') ||
          this.readProductLinkTitle(linkEl);
      ...
      extractFromDetailPage() {
        let scraped = null;
        try {
          if (window.SSAliExpressScraper?.extractProductDocument) {
            scraped = window.SSAliExpressScraper.extractProductDocument(document, location.href);
          }
        } catch (err) {
        ...
        const productId =
          scraped?.sourceId ||
          scraped?.productId ||
          (/\/item\/(\d+)(?:\.html)?/i.exec(location.pathname) || [])[1] ||
          '';
        const title =
          scraped?.title ||
          safeText(document.querySelector('[data-pl="product-title"]')) ||
        ...
        const price =
          scraped?.price
            ? [scraped.currency, scraped.price].filter(Boolean).join(' ')
            : safeText(document.querySelector('[class*="price"]'));
        const seller = scraped?.seller?.name || '';
        const shipping = scraped?.shipping?.text || '';
        const stockQty = Array.isArray(scraped?.variants) && scraped.variants.some(v => v.quantity > 0)
          ? 'In Stock'
          : '';
      ...
      insertCardOnDetailPage(wrapper) {
        ...
        const title =
          ...
          document.querySelector('[class*="title--wrap"]') ||
  ...
  async function sendRuntimeMessage(message) {
    ...
    if (response && response.ok === false) {
      throw new Error(response.error || 'Extension background rejected request');
    }
    return response;
  }
  ...
  async function handleUpload(wrapperEl, data) {
    ...
    if (isDetail) {
      ...
      try {
        ...
        await sendRuntimeMessage({ action: 'AUTO_LIST_NEW_TAB', url: targetUrl, skipSidePanelOpen: isAliExpress });
  ...
  function injectDetailCard(config) {
    ...
    if (data.supplier === 'aliexpress') {
      document.getElementById('initial-list-button-container')?.remove();
    }
