...
class AutomationController {
    ...
    async step3() {
        this.logger.info('🔍 Step 3: Branching Logic (Category OR Condition)...');
        ...
        try {
            ...
            if (pageType === 'CATEGORY') {
                return await this.handleCategoryPage();
            } else if (pageType === 'CONDITION') {
                return await this.handleConditionPage();
            } else {
                this.logger.warn('⚠️ Unknown page type in Step 3. Attempting to proceed blindly or skipping...');
                // Fallback: Check if we are already past this step?
                if (this.isStep3AlreadyComplete()) {
                    this.completedSteps.add('step3');
                    return true;
                }
                return true; // Soft fail to allow manual intervention
            }
    ...
    async detectPageType() {
        ...
        for (const selector of categoryIndicators) {
            if (selector.startsWith('.') || selector.startsWith('[') || selector.startsWith('button') || selector.startsWith('ul')) {
                if (document.querySelector(selector)) return 'CATEGORY';
            } else {
                // Text check
                if (document.body.innerText.includes(selector)) return 'CATEGORY';
            }
        }

        // Check for Condition Page indicators
        const conditionIndicators = [
            'input[type="radio"][name*="condition"]',
            '.lightbox-dialog__window', // Often wraps condition
            'h2:contains("Condition")', // heuristic
            'legend:contains("Condition")'
        ];

        // Specific DOM check for condition
        if (document.querySelector('input[type="radio"][name*="condition"]')) return 'CONDITION';

        // Text check for Condition (be careful not to match footer/header text)
        const mainContent = document.querySelector('div.main-content, div#mainContent') || document.body;
        if (mainContent.innerText.toLowerCase().includes('condition') && mainContent.innerText.toLowerCase().includes('new')) {
            return 'CONDITION';
        }
    ...
    async handleCategoryPage() {
        this.logger.info('📂 Handling Category Selection Page...');

        // Strategy: Click the FIRST suggested category
        const suggestedCategoryBtn = document.querySelector('ul button, .category-list button, button[id*="suggestion"]');

        if (suggestedCategoryBtn) {
            this.logger.info('✅ Found suggested category button. Clicking...');
            await this.domHelper.clickElement(suggestedCategoryBtn);

            // Wait for transition to next page
            await sleep(2000);

            this.completedSteps.add('step3');
            return true;
        } else {
            this.logger.warn('⚠️ On Category page but no suggested category button found.');
            return false;
        }
    }

    async handleConditionPage() {
        this.logger.info('🛠️ Handling Condition Selection Page (Existing Logic)...');

        const condition = await this.getConditionFromStorage();
        this.logger.info(`📋 Using condition value: ${condition}`);

        // Wait for lightbox if needed (sometimes it pops up)
        await sleep(1000);

        const conditionRadio = await this.findConditionRadioButton(condition);
        if (!conditionRadio) {
            this.logger.warn(`⚠️ Condition radio button not found for value: ${condition}.`);
            return true;
        }
        ...
        this.logger.info('✅ Step 3 (Condition) completed');
        this.completedSteps.add('step3');
        return true;
    }

    isStep3AlreadyComplete() {
        // If we see the main editing form (e.g. Title input, Photos section), we are past Step 3.
        if (document.querySelector('input[name="title"]') || document.querySelector('.image-uploader')) {
            this.logger.info('✅ Main listing form detected - Step 3 was already done.');
            return true;
        }
        return false;
    }
