(function() {
	...
	//#endregion
	//#region common/listing-draft.js
	/**
	...
	(function() {
		...
		*/
		function productToDraft(product, mode) {
			...
			const finalPrice = product.finalPrice != null ? parseFloat(product.finalPrice) : null;
			return {
  ...
  `;
	...
	//#endregion
	//#region common/images/adapters/amazon.image-adapter.js
	var AmazonImageAdapter = class {
		constructor() {
			this.marketplace = "amazon";
			this._scriptCache = null;
		}
		getProductId() {
			return document.querySelector("input#asin")?.value || document.querySelector("input[name=\"ASIN\"]")?.value || window.location.pathname.match(/\/dp\/([A-Z0-9]{10})/)?.[1] || window.location.pathname.match(/\/gp\/product\/([A-Z0-9]{10})/)?.[1] || null;
		}
		async getHydrationImages() {
			...
			if (Array.isArray(initial)) initial.forEach((item, idx) => {
				...
				images.push(createExtractedImage({
					...
					width: item.hiRes ? 3e3 : item.large ? 1500 : 0,
					height: item.hiRes ? 3e3 : item.large ? 1500 : 0,
		...
		async getModalImages() {
			...
			try {
				const result = await new window.FullViewImageExtractor().extractAllProductImagesFromFullView();
				const { ImageSource, ImageRole, createExtractedImage } = window.SSImageSchema;
		...
		_safeParseJson(str) {
			...
			try {
				return JSON.parse(this._repairJson(str));
			} catch {}
			return null;
		}
		_repairJson(str) {
		...
		_largestFromMap(map) {
			...
			return Object.keys(map).reduce((best, url) => {
				...
				return (Array.isArray(dimB) ? dimB[0] * dimB[1] : 0) > scoreA ? url : best;
			}, null);
		}
		_hiResFromImg(img) {
			return img.getAttribute("data-old-hires") || (() => {
				...
				if (!raw) return null;
				try {
					return this._largestFromMap(JSON.parse(raw));
				} catch {
					return null;
				}
		...
		_isExcludedItem(item) {
			...
			return cls.includes("video") || cls.includes("360") || cls.includes("spin") || html.includes("360") || item.querySelector("[class*=\"video\"],[class*=\"360\"],[class*=\"spin\"]") || cls.includes("aok-hidden") || item.style?.display === "none";
		}
		_isExcludedImg(img) {
			...
			if ([
				...
				"loading",
				"placeholder"
			].some((p) => src.includes(p) || alt.includes(p) || cls.includes(p))) return true;
	...
	if (typeof window !== "undefined") window.AmazonImageAdapter = AmazonImageAdapter;
	//#endregion
	//#region common/images/fallback/modal-click-extractor.js
	var ModalClickExtractor = class {
		constructor() {
		...
		async extract() {
			...
			if (typeof window.FullViewImageExtractor !== "undefined") try {
				return ((await new window.FullViewImageExtractor().extractAllProductImagesFromFullView())?.images || []).map((img) => createExtractedImage({
					url: img.url,
					source: ImageSource.MODAL,
					role: ImageRole.GALLERY,
					alt: img.alt || "Product Image"
		...
		async _standaloneWalk(createExtractedImage, ImageSource, ImageRole) {
			...
			clickTarget.click();
			const modal = await this._waitFor(() => this._getModal(), 7e3);
			if (!modal) return images;
			const thumbs = await this._waitFor(() => {
				const list = this._getModalThumbs(modal);
				return list.length ? list : null;
			}, 5e3);
			for (const thumb of thumbs || []) try {
				const before = this._modalSig(modal);
				(thumb.closest("button, a, [role=\"button\"]") || thumb).click();
				await this._waitFor(() => this._modalSig(modal) !== before, 5e3);
		...
		_getModal() {
			return document.querySelector("[role=\"dialog\"][aria-modal=\"true\"]") || document.querySelector("#a-popover-lgtbox") || document.querySelector(".a-modal") || document.querySelector("#ivLargeImage") || null;
		}
		_getModalThumbs(modal) {
			return Array.from(modal.querySelectorAll("img.imageThumbnail, .imageThumbnail img, #ivThumbs img, [data-index] img")).filter((img) => {
				const src = (img.src || "").toLowerCase();
				return src.includes("media-amazon") && src.includes("/images/i/") && !src.includes("play") && !src.includes("video") && !src.includes("360");
			});
		}
		_getModalMain(modal) {
			...
			return imgs.reduce((best, cur) => cur.clientWidth * cur.clientHeight > best.clientWidth * best.clientHeight ? cur : best, imgs[0]);
		}
		_modalSig(modal) {
		...
		_hiResFromImg(img) {
			...
			if (dyn) try {
				...
				return Object.keys(map).reduce((best, url) => {
					...
					return (map[url]?.[0] || 0) * (map[url]?.[1] || 0) > a ? url : best;
				}, null);
			} catch {}
		...
		_maxRes(url) {
			if (!url) return url;
			if (url.includes("._")) return url.split("._")[0] + (url.match(/\.(jpg|jpeg|png|webp)$/i)?.[0] || ".jpg");
		...
		_waitFor(fn, timeoutMs = 5e3, intervalMs = 50) {
			return new Promise((resolve) => {
				const start = Date.now();
				const tick = () => {
					try {
	...
	if (typeof window !== "undefined") window.SSModalClickExtractor = ModalClickExtractor;
	//#endregion
	//#region common/amazon_image_extractor.js
	var ComprehensiveAmazonImageExtractor = class {
		constructor() {
			this.images = /* @__PURE__ */ new Map();
			this.altMap = /* @__PURE__ */ new Map();
			this.highQualityImages = [];
			this.currentASIN = null;
			this.extractedBaseUrls = /* @__PURE__ */ new Set();
		}
		getCurrentASIN() {
			if (this.currentASIN) return this.currentASIN;
			this.currentASIN = document.querySelector("input#asin")?.value || document.querySelector("input[name=\"ASIN\"]")?.value || window.location.pathname.match(/\/dp\/([A-Z0-9]{10})/)?.[1] || window.location.pathname.match(/\/gp\/product\/([A-Z0-9]{10})/)?.[1] || "";
			console.log("📦 Current product ASIN:", this.currentASIN);
			return this.currentASIN;
		}
		sanitizeAltText(text) {
			if (!text) return "Product Image";
			return text.replace(/\b(amazon|prime|alexa|kindle|fire tv|echo|basics)\b/gi, "").replace(/\s+/g, " ").trim() || "Product Image";
		}
		async extractAllImages() {
			...
			const methods = [
				{
					name: "Modal Extraction",
					fn: () => this.extractFromImageModal()
				},
				{
					name: "altImages Gallery",
					fn: () => this.extractFromAltImages()
				},
				{
					name: "ImageBlock Data",
					fn: () => this.extractFromImageBlock()
				},
				{
					name: "Script JSON Data",
					fn: () => this.extractFromScriptData()
				},
				{
					name: "Fallback Selectors",
					fn: () => this.extractFallback()
				}
			];
		...
		async extractFromImageModal() {
			...
			if (!mainImage) return;
			document.querySelector("#ivLargeImage, .iv-large-image, #imageBlockContainer");
			...
			if (imageViewer) {
				...
				for (const img of allImgs) if (!this.isExcludedImage(img)) this.extractHighResFromElement(img);
			}
		}
		async extractFromAltImages() {
			...
			for (const item of allItems) {
				if (this.isExcludedThumbnail(item)) continue;
				const img = item.querySelector("img");
				if (!img) continue;
				if (this.isExcludedImage(img)) continue;
				if (this.extractHighResFromElement(img)) imageCount++;
		...
		extractFromImageBlock() {
			...
			for (const sel of [
				...
				if (img && !this.isExcludedImage(img)) this.extractHighResFromElement(img);
			}
			imageBlock.querySelectorAll("img[data-old-hires], img[data-a-dynamic-image]").forEach((img) => {
				if (!this.isExcludedImage(img)) this.extractHighResFromElement(img);
			});
		}
		extractFromScriptData() {
			...
			for (const script of scripts) {
				...
				if (content.includes("colorImages") || content.includes("ImageBlockATF")) this.parseColorImagesData(content);
			}
		}
		parseColorImagesData(content) {
			for (const pattern of [
				...
				if (match) try {
					...
					if (Array.isArray(images)) images.forEach((item) => {
						let url = item.hiRes || item.large;
						if (!url && item.main) {
							if (typeof item.main === "string") url = item.main;
							else if (typeof item.main === "object") url = this.getLargestFromDynamicMap(item.main);
						}
		...
		extractUrlsWithRegex(text) {
			...
			while ((match = urlPattern.exec(text)) !== null) {
				...
				if (this.isValidProductImageUrl(url)) this.addImageWithDedup(url, "Product Image");
			}
		}
		extractFallback() {
			for (const sel of [
		...
		extractHighResFromElement(img) {
			...
			if (!highResUrl) {
				...
				if (dynamicData) try {
					...
					if (highResUrl) console.log(`    ✓ Found from dynamic-image: ${highResUrl.substring(0, 60)}...`);
				} catch (e) {
					console.warn("    Failed to parse data-a-dynamic-image");
				}
			}
			if (!highResUrl && img.src && this.isValidProductImageUrl(img.src)) highResUrl = img.src;
			if (highResUrl) return this.addImageWithDedup(highResUrl, altText);
			return false;
		}
		getLargestFromDynamicMap(imageMap) {
			...
			return urls.reduce((best, url) => {
				...
				return this.getResolutionScore(imageMap[url]) > sizeA ? url : best;
			}, null);
		}
		getResolutionScore(dimensions) {
			if (Array.isArray(dimensions) && dimensions.length >= 2) return (dimensions[0] || 0) * (dimensions[1] || 0);
			return 0;
		}
		addImageWithDedup(url, altText) {
			const baseUrl = this.getBaseImageUrl(url);
			if (this.extractedBaseUrls.has(baseUrl)) return false;
		...
		isExcludedThumbnail(item) {
			...
			if (classList.includes("video") || item.querySelector(".videoThumbnail, [class*=\"video\"], .a-video")) return true;
			if (classList.includes("360") || classList.includes("spin") || innerHTML.includes("360") || item.querySelector("[class*=\"360\"], [class*=\"spin\"]")) return true;
			if (classList.includes("aok-hidden") || item.style.display === "none") return true;
			return false;
		}
		isExcludedImage(img) {
			if (!img) return true;
			...
			if (src.includes("video") || src.includes("play-button") || src.includes("play_icon") || src.includes("play-icon") || classList.includes("video")) return true;
			if (src.includes("360") || src.includes("spin") || alt.includes("360") || classList.includes("360")) return true;
			if (src.includes("sprite") || src.includes("icon") || src.includes("transparent-pixel") || src.includes("spacer") || src.includes("loading") || src.includes("placeholder")) return true;
			const width = img.naturalWidth || img.width || 0;
			const height = img.naturalHeight || img.height || 0;
			if (width > 0 && width < 30 && height > 0 && height < 30) return true;
			return false;
		}
		isValidProductImageUrl(url) {
			...
			if (![
				...
				"images-eu.ssl-images-amazon.com",
				"images-fe.ssl-images-amazon.com"
			].some((domain) => url.includes(domain))) return false;
		...
		forceMaxResolution(originalUrl) {
			...
			if (baseMatch) url = `${baseMatch[1]}._AC_SL3000_.${baseMatch[2]}`;
			else {
				[
					/_AC_S[XLYS]\d+_/gi,
					/_AC_U[SXYL]\d+_/gi,
					...
					if (pattern.test(url)) url = url.replace(pattern, "_AC_SL3000_");
				});
				if (!url.includes("_AC_SL3000_") && !url.includes("._")) {
					const extMatch = url.match(/\.(jpg|jpeg|png|webp)$/i);
					if (extMatch) url = url.replace(extMatch[0], `._AC_SL3000_${extMatch[0]}`);
				}
			}
			return url;
		}
		transformToMaxResolution() {
			...
			originalUrls.forEach((url) => {
				...
				if (!transformed.has(baseUrl)) transformed.set(baseUrl, {
					url: maxRes,
					...metadata
				});
			});
	...
	if (typeof window !== "undefined") window.ComprehensiveAmazonImageExtractor = ComprehensiveAmazonImageExtractor;
	//#endregion
	//#region common/full_view_image_extractor.js
	var FullViewImageExtractor = class {
		constructor() {
			this.results = {
				productTitle: "",
				totalImages: 0,
			...
			this._seenBaseIds = /* @__PURE__ */ new Set();
		}
		/**
		...
		*/
		async extractAllProductImagesFromFullView() {
			...
			try {
				...
				if (!clickable) throw new Error("Could not find main product image to click");
				clickable.click();
				console.log("✓ Clicked main image, waiting for modal...");
				const modalRoot = await this.waitFor(() => this.getModalRoot(), { timeoutMs: 7e3 });
				if (!modalRoot) throw new Error("Modal did not appear");
				...
				for (let i = 0; i < thumbList.length; i++) try {
					...
					if (!imageUrl) imageUrl = thumb.src || thumb.getAttribute("data-src") || null;
					if (imageUrl) imageUrl = this.getMaxResolutionUrl(imageUrl);
					if (!imageUrl) {
		...
		getModalRoot() {
			return document.querySelector("[role=\"dialog\"][aria-modal=\"true\"]") || document.querySelector("#a-popover-lgtbox") || document.querySelector(".a-modal") || document.querySelector("#ivLargeImage")?.closest("[role=\"dialog\"], .a-modal, #a-popover-lgtbox") || document.querySelector("#ivLargeImage") || null;
		}
		getModalThumbnails(modalRoot) {
			return Array.from(modalRoot.querySelectorAll("img.imageThumbnail, .imageThumbnail img, [role=\"button\"][aria-label*=\"image\"] img, img[data-index], #ivThumbs img")).filter((el) => {
		...
		getModalMainImage(modalRoot) {
			...
			return imgs.reduce((best, cur) => {
				...
				return (cur?.clientWidth || 0) * (cur?.clientHeight || 0) > a ? cur : best;
			}, imgs[0]);
		}
		getCurrentModalImageSignature(modalRoot) {
			const img = this.getModalMainImage(modalRoot);
			if (!img) return null;
			return img.getAttribute("data-old-hires") || img.getAttribute("data-a-dynamic-image") || img.getAttribute("src") || null;
		}
		extractHiResUrlFromImg(imgEl) {
			...
			if (dynamicData) try {
				...
				if (urls.length > 0) return urls.reduce((best, url) => {
					const sizeA = (parsed[best]?.[0] || 0) * (parsed[best]?.[1] || 0);
					return (parsed[url]?.[0] || 0) * (parsed[url]?.[1] || 0) > sizeA ? url : best;
				});
			} catch (e) {}
		...
		*/
		getMaxResolutionUrl(url) {
			if (!url) return url;
			let maxUrl = url;
			if (maxUrl.includes("._")) maxUrl = maxUrl.split("._")[0] + (maxUrl.match(/\.(jpg|jpeg|png|webp)$/i)?.[0] || ".jpg");
			[
				{
					pattern: /_AC_S[XY]\d+_/g,
					replacement: "_AC_SL3000_"
				},
				{
					pattern: /_AC_U[SXYL]\d+_/g,
					replacement: "_AC_SL3000_"
				},
				{
					pattern: /_SS\d+_/g,
					replacement: "_SL3000_"
				},
				{
					pattern: /_SX\d+_/g,
					replacement: "_SL3000_"
			...
			if (!maxUrl.includes("_SL3000_") && !maxUrl.includes("_AC_SL3000_")) {
				...
				if (ext) maxUrl = maxUrl.replace(ext, `._AC_SL3000_${ext}`);
			}
			return maxUrl;
		}
		getBaseImageId(url) {
		...
		*/
		waitFor(fn, { timeoutMs = 5e3, intervalMs = 50 } = {}) {
			const start = Date.now();
			return new Promise((resolve) => {
				const tick = () => {
					try {
	...
	if (typeof window !== "undefined") window.FullViewImageExtractor = FullViewImageExtractor;
	//#endregion
	//#region content_scripts/amazon-variant-scraper.js
	/**
	...
	*/
	(function() {
			...
			for (let i = 0; i < raw.length; i++) {
				...
				if (ch === "\"" && !prevBackslash) {
					if (!inStr) {
						...
						const next = raw[j];
						let closes = next === void 0 || next === "," || next === "]" || next === "}";
						if (!closes && next === ":") {
							let k = j + 1;
							while (k < raw.length && /\s/.test(raw[k])) k++;
							const ac = raw[k];
							closes = ac === "\"" || ac === "{" || ac === "[" || ac === "t" || ac === "f" || ac === "n" || ac === "-" || ac === void 0 || ac >= "0" && ac <= "9";
						}
						if (closes) {
							inStr = false;
							out += ch;
		...
		function _robustParse(raw, maxTries = 30) {
			let s = raw, lastPos = -1;
			for (let attempt = 0; attempt < maxTries; attempt++) try {
				return JSON.parse(s);
			} catch (e) {
		...
		*/
		function getWindowScriptData() {
			const PATTERNS = [
				{
					sig: "title",
					re: /jQuery\.parseJSON\(\s*(['"])((?:\\.|(?!\1).)*?)\1\s*\)/
				},
				{
					sig: "twister-js-init-dpx-data",
					re: /dataToReturn\s*=\s*(\{[\s\S]*?\});/
				},
				{
					sig: "ImageBlockBTF",
					re: /jQuery\.parseJSON\(\s*(['"])((?:\\.|(?!\1).)*?)\1\s*\)/
				...
				{
					sig: "ImageBlockATF",
					re: /P\.when\('A'\)\.register\("ImageBlockATF",\s*function\(A\)\{\s*var data\s*=\s*(\{[\s\S]*?\});/
				}
			];
			const found = [
				null,
			...
			document.querySelectorAll("script").forEach((script) => {
				...
				PATTERNS.forEach((pat, idx) => {
					...
					if (!m) return;
					const raw = m[2] !== void 0 ? m[2] : m[1];
					if (!raw) return;
					try {
						found[idx] = _parseWindowJson(raw);
					} catch (_) {}
				});
			});
		...
		function waitForMessage(key, timeoutMs = 1500) {
			return new Promise((resolve, reject) => {
				...
				const timer = setTimeout(() => {
					window.removeEventListener("message", handler);
					reject(/* @__PURE__ */ new Error("Timeout waiting for " + key));
				}, timeoutMs);
		...
		function _cleanPriceText(str) {
			...
			if (match) {
				...
				else if (lastComma !== -1) if (rawNum.length - lastComma === 4) decimalSep = ".";
				else decimalSep = ",";
				else if (lastDot !== -1) if (rawNum.length - lastDot === 4) decimalSep = ",";
				else decimalSep = ".";
				let priceStr = "";
				for (let i = 0; i < rawNum.length; i++) {
					const ch = rawNum[i];
					if (ch >= "0" && ch <= "9") priceStr += ch;
					else if (ch === decimalSep) priceStr += ".";
				}
		...
		function checkStock() {
			...
			return [
				...
				/^auf lager/,
				/^nur noch \d+ auf lager/,
				/^versandfertig in/,
				/^en stock/,
				/^solo quedan? \d+ en stock/,
				/^op voorraad/,
				/^nog maar \d+ op voorraad/,
				/^il ne reste plus que \d+/,
				/^disponibilit/
			].some((p) => p.test(txt));
		}
		...
		async function getVariations(windowData, minQty, allowLowQty) {
			...
			varContainers.forEach((container, l) => {
				const selectOpts = Array.from(container.querySelectorAll("option")).filter((opt) => !(opt.getAttribute("data-a-id") || "").includes("-1")).map((opt) => "#" + opt.getAttribute("data-a-id"));
			...
			for (const asin in t.asinToDimensionIndexMap) {
				...
				for (let h = 0; h < dimIndices.length; h++) {
					...
					if (typeof optEntry === "string") {
						if (container && container.querySelectorAll("option").length > 0) {
			...
			const visualDimIndices = (p.visualDimensions || []).map((v) => dims.findIndex((d) => d === v)).filter((i) => i >= 0);
			for (const asin in t.dimensionValuesDisplayData) try {
				let price = 999, currency = "USD", quantity = 0;
				try {
					const msgPromise = waitForMessage(`ss-buybox-${asin}`, 3e3);
					setTimeout(() => {
						window.postMessage({
							from: "ss-amazon-cs",
							action: "getBuybox",
					...
					if (bbData.quantity) quantity = bbData.quantity;
					else quantity = 1;
					const group = JSON.parse(bbData.buyboxRaw).desktop_buybox_group_1[0];
					if (!group) quantity = 0;
					else {
				...
				const attrs = {};
				if (Array.isArray(displayData)) displayData.forEach((val, i) => {
				...
				let img, imgProp;
				const colorImagesLower = Object.fromEntries(Object.entries(colorImages).map(([k, v]) => [k.toLowerCase(), v]));
				const ciKey = colorImagesLower[colorKey.toLowerCase()] ? colorKey.toLowerCase() : Object.keys(colorImagesLower).find((k) => k.startsWith(colorKey.split(" ")[0].toLowerCase()) || colorKey.toLowerCase().startsWith(k.split(" ")[0]));
		...
		function getImages(windowData) {
			...
			if (ci && Array.isArray(ci.initial)) ci.initial.filter((img) => img.hiRes).forEach((img) => push(img.hiRes));
			if (p && p.colorImages) Object.keys(p.colorImages).forEach((key) => {
		...
		function getImagesSingle(windowData) {
			...
			if (ci && Array.isArray(ci.initial)) {
				...
				if (images.length === 0) ci.initial.forEach((img) => push(img.large || img.thumb || null));
			}
			if (images.length === 0) {
				document.querySelectorAll("#altImages li img, #imageBlock img").forEach((img) => {
					let src = img.getAttribute("data-old-hires") || img.src || "";
		...
		async function scrapeSingleProduct() {
			if (document.querySelector("form[action*=\"validateCaptcha\"]") || document.body && document.body.innerText.includes("Type the characters you see in this image")) throw new Error("Amazon CAPTCHA detected — please solve it and try again");
			...
			if (variationData && variationData.asinToDimensionIndexMap && currentAsin) {
				...
				if (dimIndices) variationData.dimensions.forEach((dim, i) => {
			...
			const specs = {};
			document.querySelectorAll("#productOverview_feature_div tr, #productDetails_expanderTables_depthLeftSections tr, #productDetails_techSpec_section_1 tr").forEach((row) => {
		...
		async function scrapeProductWithVariants(options) {
			...
			const windowData = getWindowScriptData();
			const { variationData, jqueryData } = windowData;
			const asin = document.querySelector("input#asin")?.value || window.location.pathname.match(/\/(?:dp|gp\/aw\/d)\/([A-Z0-9]{10})/)?.[1] || "";
			...
			return {
				...
				currency: selected ? resolveCurrency(selected.currency || "$") : "USD",
	...
	//#endregion
	//#region content_scripts/amazon-scraper-v2.js
	/**
	...
	*/
	(function() {
		...
		*/
		...
		function _parseFragment(raw) {
			...
			try {
				...
				const swapped = raw.replace(/\\'/g, SENT).replace(/"/g, "\\\"").replace(/'/g, "\"").split(SENT).join("'");
		...
		*/
		...
		function _cleanSpecPair(key, val) {
			...
			if (!k || !v) return null;
			if (/^(customer reviews|best sellers rank|asin|date first available)$/i.test(k)) return null;
		...
		*/
		function _parseTwisterFields(twisterText) {
			...
			const fields = {
				...
				parentAsin: (() => {
					const v = _parseFragment(_extractKeyedValue(twisterText, "parentAsin"));
					return typeof v === "string" ? v : null;
				})()
			};
			if (fields.dimensionToAsinMap && typeof fields.dimensionToAsinMap === "object") {
				const inv = {};
				for (const dimKey of Object.keys(fields.dimensionToAsinMap)) inv[fields.dimensionToAsinMap[dimKey]] = dimKey.split("_").map(Number);
				fields.asinToDimensionIndexMap = inv;
			} else fields.asinToDimensionIndexMap = null;
			return fields;
		}
		function _cleanPriceText(str) {
			if (!str) return null;
		...
		*/
		function _collectSpecsDom() {
			...
			const addPair = (k, v) => {
				...
				if (pair && specs[pair[0]] === void 0) specs[pair[0]] = pair[1];
			};
			try {
			...
			try {
				document.querySelectorAll("#detailBullets_feature_div li .a-list-item").forEach((li) => {
					...
					addPair(key, li.textContent.replace(bold.textContent, ""));
				});
			} catch (_) {}
			try {
				const brand = document.querySelector("#bylineInfo");
				if (brand && specs["Brand"] === void 0) {
		...
		function _collectImages(texts, colorImagesMap) {
			...
			try {
				...
				if (atf) {
					...
					(ci.initial || ci[Object.keys(ci)[0]] || []).forEach((img) => push(img.hiRes || img.large || img.thumb));
				}
			} catch (_) {}
			try {
		...
		function _attrsForAsin(tw, asin) {
			...
			const labelFor = (dim, i) => tw.variationDisplayLabels && tw.variationDisplayLabels[dim] || Array.isArray(tw.dimensionsDisplay) && tw.dimensionsDisplay[i] || dim;
			if (Array.isArray(display)) display.forEach((val, i) => {
				const dim = dims[i];
		...
		function _imageForVariant(tw, attrs, colorImagesMap) {
			try {
				const dims = Array.isArray(tw.dimensions) ? tw.dimensions : [];
				const labels = (Array.isArray(tw.visualDimensions) && tw.visualDimensions.length ? tw.visualDimensions : dims.length === 1 ? dims : dims.filter((d) => /colou?r/i.test(d))).map((v) => {
					const i = dims.indexOf(v);
					return tw.variationDisplayLabels && tw.variationDisplayLabels[v] || Array.isArray(tw.dimensionsDisplay) && i >= 0 && tw.dimensionsDisplay[i] || v;
				}).filter(Boolean);
				let colorKey = "";
				labels.forEach((label) => {
					if (attrs[label]) colorKey += attrs[label].productName + " ";
				});
				...
				const lower = {};
				Object.keys(colorImagesMap || {}).forEach((k) => {
					lower[k.toLowerCase()] = colorImagesMap[k];
				});
				const lk = colorKey.toLowerCase();
				const matchKey = lower[lk] ? lk : Object.keys(lower).find((k) => k.startsWith(lk.split(" ")[0]) || lk.startsWith(k.split(" ")[0]));
				const first = matchKey && lower[matchKey] && lower[matchKey][0];
				return {
					img: first ? _stripSizeModifier(first.hiRes || first.large || first.thumb || null) : void 0,
		...
		*/
		async function _clickThroughVariants(tw) {
			...
			for (const asin in tw.asinToDimensionIndexMap) {
				...
				for (let h = 0; h < dimIndices.length; h++) try {
					const remappedDimKey = s[h];
		...
		async function _fetchBuyboxForAsin(asin) {
			const msgPromise = _waitForMessage(`ss-buybox-${asin}`, 3e3);
			setTimeout(() => {
				window.postMessage({
					from: "ss-amazon-cs",
					action: "getBuybox",
		...
		async function scrapeProductWithVariants(options) {
			...
			let colorImagesMap = {};
			try {
				for (const t of texts) if (t.includes("colorImages")) {
					const parsed = _extractColorImages(t.includes("jQuery.parseJSON") ? t.replace(/\\'/g, "'") : t);
					if (parsed && Object.keys(parsed).length > 0) for (const [k, val] of Object.entries(parsed)) {
						if (k === "initial" && colorImagesMap[k]) continue;
						if (!colorImagesMap[k] || val.length > colorImagesMap[k].length) colorImagesMap[k] = val;
					}
				}
			} catch (_) {}
			const images = _collectImages(texts, colorImagesMap);
			const inStock = _checkStock();
			const hasTwister = tw.dimensionValuesDisplayData && Object.keys(tw.dimensionValuesDisplayData).length > 1 && Array.isArray(tw.dimensions) && tw.dimensions.length > 0;
			...
			if (!hasTwister) variants = [{
				...
				if (qualified.length === 0) if (variants.length > 0) {
					if (!allowLowQty) throw new Error("This product is low on quantity");
				} else throw new Error("Zero variations found");
				else variants = qualified;
			}
			const selected = variants.find((v) => v.quantity > 0) || variants[0];
			return {
		...
		async function scrapeSingleProduct() {
			...
			let attrs = {};
			try {
				if (tw.asinToDimensionIndexMap && base.currentAsin && tw.asinToDimensionIndexMap[base.currentAsin]) attrs = _attrsForAsin(tw, base.currentAsin);
			} catch (_) {}
			if (Object.keys(attrs).length === 0) try {
				const selColor = document.querySelector("#variation_color_name .selection, #variation_color_name .a-color-base")?.textContent?.trim();
				const selSize = document.querySelector("#variation_size_name .selection, #variation_size_name .a-color-base")?.textContent?.trim();
				if (selColor) attrs["Color"] = { productName: selColor };
				if (selSize) attrs["Size"] = { productName: selSize };
	...
	//#endregion
	//#region content_scripts/amazon_injector.js
	...
	var waitForElement = (selector, timeout = 5e3) => {
		return new Promise((resolve, reject) => {
			const cached = cache.get(`element_${selector}`);
			if (cached && document.contains(cached)) return resolve(cached);
			const existing = document.querySelector(selector);
			if (existing) {
	...
	var scrapeCompleteProductData = () => {
		...
		const getAllElText = (selector) => {
			return Array.from(document.querySelectorAll(selector)).map((el) => el.innerText.trim()).filter((text) => text);
		};
		const productData = {
			asin: document.querySelector("input#asin")?.value || window.location.pathname.match(/\/dp\/([A-Z0-9]{10})/)?.[1] || "",
			...
			allImages: Array.from(document.querySelectorAll("#altImages li img, #imageBlock img")).map((img) => {
				let src = img.getAttribute("data-old-hires") || img.getAttribute("data-a-dynamic-image") || img.src;
				if (src && src.includes("._")) src = src.replace(/\._[A-Z0-9,]+_\./, ".");
				return src;
			}).filter((src) => src && !src.includes("base64") && !src.includes("transparent-pixel")).filter((src, index, self) => self.indexOf(src) === index),
			...
			rating: getElText("#acrPopover")?.match(/[\d.]+/)?.[0] || getElText(".a-icon-star")?.match(/[\d.]+/)?.[0] || "",
		...
		if (techSpecTable) techSpecTable.querySelectorAll("tr").forEach((row) => {
			const key = row.querySelector("th")?.innerText.trim();
			const value = row.querySelector("td")?.innerText.trim();
			if (key && value) productData.specifications[key] = value;
		});
		const detailBullets = document.querySelector("#detailBullets_feature_div");
		if (detailBullets) detailBullets.querySelectorAll("li span.a-list-item").forEach((item) => {
		...
		if (prodDetails) prodDetails.querySelectorAll("tr").forEach((row) => {
			const cells = row.querySelectorAll("th, td");
			if (cells.length >= 2) {
				const key = cells[0].innerText.trim();
				const value = cells[1].innerText.trim();
				if (key && value && !productData.specifications[key]) productData.specifications[key] = value;
			}
		});
		const aplusDiv = document.querySelector("#aplus_feature_div") || document.querySelector("#aplus") || document.querySelector("#aplusBrandStory_feature_div");
		if (aplusDiv) productData.aplusContent = {
			text: aplusDiv.innerText.trim().substring(0, 5e3),
			images: Array.from(aplusDiv.querySelectorAll("img")).map((img) => img.src).filter((src) => src && !src.includes("transparent-pixel")),
			hasContent: true
		};
		const variantSelectors = document.querySelectorAll("#variation_size_name li, #variation_color_name li");
		if (variantSelectors.length > 0) productData.variants = Array.from(variantSelectors).map((li) => ({
			name: li.getAttribute("title") || li.innerText.trim(),
			selected: li.classList.contains("selected") || li.classList.contains("swatchSelect")
		}));
		const missingFields = ["asin", "title"].filter((field) => !productData[field]);
		if (missingFields.length > 0) console.warn("[Scraper] Missing required fields:", missingFields);
	...
	var injectUI = async ({ fromSidebar = false, sidebarImages = [] } = {}) => {
		...
		await chrome.storage.local.get("saasToken");
		...
		try {
			...
			if (fromSidebar) renderGalleryFromUrls(sidebarImages);
				...
				waitForElement("#snipe-title-btn", 2e3).then((btn) => {
	...
	var scrapeAndStoreProductData = async () => {
		console.log("📦 [ProductScraper] Starting comprehensive product data scrape...");
		try {
			if ((await chrome.storage.local.get("panelSource")).panelSource === "sidebar") {
	...
	var scrapeFullProductData = () => {
		...
		for (const selector of [
			...
			if (bullets.length > 0) {
				bullets.forEach((bullet) => {
					...
					if (text && text.length > 10 && !data.bulletPoints.includes(text)) data.bulletPoints.push(text);
				});
		...
		for (const selector of [
			...
			if (descEl) {
				...
				if (text && text.length > 50) {
					data.description = text.substring(0, 2e3);
		...
		if (breadcrumbEl) {
			...
			crumbs.forEach((crumb) => {
				...
				if (text && text !== "›" && text.length > 1) categories.push(text);
			});
		...
		document.querySelectorAll("#productDetails_techSpec_section_1 tr, #productDetails_detailBullets_sections1 tr, #detailBullets_feature_div li, .prodDetTable tr").forEach((row) => {
			...
			if (labelEl && valueEl) {
				...
				if (label && value && label.length < 50 && value.length < 200) data.specifications[label] = value;
			}
		});
		if (Object.keys(data.specifications).length > 0) console.log("📝 Specifications scraped:", Object.keys(data.specifications).length, "items");
		const asinMatch = window.location.href.match(/\/dp\/([A-Z0-9]{10})/i) || window.location.href.match(/\/product\/([A-Z0-9]{10})/i);
		if (asinMatch) {
	...
	var scrapeProductDetails = () => {
		...
		if (detailBullets) detailBullets.querySelectorAll("li").forEach((item) => {
			...
			if (labelElement && valueElement) {
				...
				if (label.includes("product dimensions")) details.dimensions = value;
				else if (label.includes("item model number")) details.model = value;
				else if (label.includes("manufacturer")) details.brand = value;
				else if (label.includes("color")) details.color = value;
				else if (label.includes("weight")) details.weight = value;
				else if (label.includes("height")) details.height = value;
			}
		});
		document.querySelectorAll("table[id*=\"productDetails\"], #productDetails_techSpec_section_1, #productDetails_techSpec_section_2").forEach((table) => {
		...
		document.querySelectorAll("[data-feature-name*=\"dimension\"], [data-feature-name*=\"weight\"], [data-feature-name*=\"color\"]").forEach((section) => {
		...
		if (descriptionElement) details.description = descriptionElement.innerText.trim();
		if (!details.weight || !details.dimensions || !details.brand) {
			...
			if (!details.brand) {
				...
				if (brandMatch) {
					...
					if (brandStr.length > 0 && brandStr.length < 50) details.brand = brandStr;
				}
			}
		}
		return details;
	};
	var AmazonImageExtractor = class {
		constructor() {
		...
		async extractFromFullViewModal() {
			...
			try {
				...
				if (!mainImage) throw new Error("Main product image not found");
				console.log("  Clicking main image to open modal...");
				mainImage.click();
				await this.wait(500);
				if (!document.querySelector(".a-modal-scroller, #ivLargeImage")) throw new Error("Modal did not open");
				console.log("  ✓ Modal opened successfully");
				const thumbnails = Array.from(document.querySelectorAll(".ivThumb"));
				if (thumbnails.length === 0) throw new Error("No thumbnails found in modal");
				console.log(`  Found ${thumbnails.length} thumbnails to process`);
				for (let i = 0; i < thumbnails.length; i++) try {
					const thumb = thumbnails[i];
					if (thumb.querySelector("video") || thumb.classList.contains("video") || thumb.getAttribute("aria-label")?.toLowerCase().includes("video")) {
					...
					if (largeImage) {
						...
						if (dataOldHires) imageUrl = dataOldHires;
		...
		async extractComprehensive() {
			...
			mainImageBlock.querySelectorAll("img[data-old-hires], img[data-a-dynamic-image]").forEach((img) => {
				const altText = img.alt || "";
				...
				if (img.dataset.aDynamicImage) try {
					...
					for (const [url, dimensions] of Object.entries(imageData)) if (url && this.isValidImageUrl(url)) {
						this.images.add(url);
						if (altText) this.altMap.set(url, altText);
		...
		async extractFallback() {
			...
			document.querySelectorAll("#altImages, #imageBlock, #main-image-container").forEach((container) => {
		...
		transformToHighRes() {
			...
			originalUrls.forEach((url) => {
				...
				if (this.altMap.has(url)) this.altMap.set(highResUrl, this.altMap.get(url));
		...
		getHighResUrl(originalUrl) {
			...
			if (highResUrl.includes("._")) highResUrl = `${highResUrl.split("._")[0]}${highResUrl.match(/\.(jpg|jpeg|png|webp)/i)?.[0] || ".jpg"}`;
			[
				{
					pattern: /\._[A-Z0-9]+_\./g,
				...
				{
					pattern: /_AC_SX300_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_SX500_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_SX1000_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_SY90_/g,
				...
				{
					pattern: /_AC_SY300_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_SY500_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_SY1000_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_US\d+_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_U\d+_/g,
				...
				{
					pattern: /_AC_UL\d+_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_UX\d+_/g,
					replacement: "_AC_SL1500_"
				},
				{
					pattern: /_AC_UY\d+_/g,
					replacement: "_AC_SL1500_"
				}
			].forEach((transform) => {
		...
		getImageAlt(url) {
			if (this.altMap.has(url)) return this.sanitizeAltText(this.altMap.get(url));
		...
		isValidImageUrl(url) {
			...
			const hasValidFormat = [
				...
				".png",
				".webp"
			].some((format) => url.toLowerCase().includes(format));
	...
	var extractor = new AmazonImageExtractor();
	...
	async function runV2Comparison(oldImages, context = "unknown") {
		...
		if (typeof window.AmazonImageAdapter === "undefined" || typeof window.SSExtractionEngine === "undefined" || typeof window.SSImageSchema === "undefined") {
		...
		try {
			...
			const oldOpenedModal = (oldImages || []).some((img) => img.source === "modal" || img._source === "modal");
			v2Images = await engine.extract(adapter, {
				minConfidentImages: 1,
				maxModalFallback: oldOpenedModal
			});
		...
		const oldIds = new Set((oldImages || []).map((img) => normalizer ? normalizer.getBaseId(img.url || img, "amazon") : img.url || img).filter(Boolean));
		const v2Ids = new Set(v2Images.map((img) => img.id || (normalizer ? normalizer.getBaseId(img.url, "amazon") : img.url)).filter(Boolean));
		const missingInV2 = [...oldIds].filter((id) => !v2Ids.has(id));
		const extraInV2 = [...v2Ids].filter((id) => !oldIds.has(id));
		...
		const report = {
			context,
			timestamp: (/* @__PURE__ */ new Date()).toISOString(),
			asin: extractor.currentASIN || window.location.pathname.match(/\/dp\/([A-Z0-9]{10})/)?.[1] || "unknown",
			...
			modalUsed,
			v2Error,
			verdict: v2Error ? "V2_ERROR" : v2Count >= oldCount ? extraInV2.length > 0 ? "V2_BETTER" : "V2_EQUAL" : "V2_WORSE"
		};
	...
	var scrapeAndDisplayInitialTitle = () => {
		...
		const titleDisplay = document.getElementById("ai-generated-title");
		const titleCounter = document.getElementById("ai-title-counter");
		if (titleDisplay) titleDisplay.innerText = originalTitle;
	...
	var renderGalleryFromUrls = async (urls = []) => {
		...
		else for (let i = 0; i < allImages.length; i++) {
			const { url } = allImages[i];
			try {
				const processedUrl = i === 0 && autoWatermarkEnabled ? await processImageTo1600x1600(url) : await processImageOriginal(url);
				...
				for (let batchStart = 0; batchStart < allImages.length; batchStart += BATCH_SIZE) {
					...
					for (let i = batchStart; i < batchEnd; i++) batchPromises.push((async () => {
						...
						try {
							...
							if (i === 0 && autoWatermarkEnabled) processedImageUrl = await processImageTo1600x1600(imageInfo.url);
							else processedImageUrl = await processImageOriginal(imageInfo.url);
							return {
								imageInfo,
								processedImageUrl,
								index: i
	...
	var processImageOriginal = (imageUrl) => {
		return new Promise((resolve, reject) => {
	...
	var addEventListenersToPanel = () => {
            ...
            `;
				try {
					...
					if (bgResp.titles && bgResp.titles.length > 0) {
						...
						if (titlesToSave.length > 0 && typeof window.SSListingDraft !== "undefined") window.SSListingDraft.patchDraft({
						...
						else console.error("❌ UIHelper.renderInlineTitles is not available globally");
						titlesToSave.forEach((titleValue, index) => {
							...
							if (textarea) {
								...
								if (counter) counter.textContent = (titleValue || "").length;
		...
		if (generateDescriptionBtn) {
			generateDescriptionBtn.addEventListener("click", async () => {
				...
				try {
					...
					console.log("═══════════════════════════════════════════════════════");
					if (!productData?.title) throw new Error("No product title found. Open an Amazon product page (not search results).");
					if (descriptionPreviewEl) descriptionPreviewEl.innerHTML = `
                        <div class="description-placeholder">
                            <div class="spinner-small"></div>
					...
					if (descriptionPreviewEl) descriptionPreviewEl.innerHTML = lastGeneratedDescription;
					...
					console.log("✅ AI Description generated:", {
						provider: bgResp.provider,
						model: bgResp.model,
						length: bgResp.length
					});
				} catch (err) {
					console.error("═══════════════════════════════════════════════════════");
                    ...
                    `;
					else if (isRateLimitError) displayHtml = `
                        <div style="text-align: left; padding: 15px;">
                    ...
                    `;
					else if (isCreditsError) displayHtml = `
                        <div style="text-align: left; padding: 15px;">
                    ...
                    `;
					if (descriptionPreviewEl) descriptionPreviewEl.innerHTML = displayHtml;
		...
		const syntaxHighlightJSON = (json) => {
			...
			return json.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/("(\\u[a-zA-Z0-9]{4}|\\[^u]|[^\\"])*"(\s*:)?)/g, (match) => {
				let cls = "json-string";
				if (/:$/.test(match)) {
					cls = "json-key";
					match = match.replace(/:$/, "") + ":";
			...
			}).replace(/\b(true|false)\b/g, "<span class=\"json-boolean\">$1</span>").replace(/\bnull\b/g, "<span class=\"json-null\">null</span>").replace(/\b(\d+)\b/g, "<span class=\"json-number\">$1</span>");
		};
		const updateScrapeSummary = (data) => {
			if (!scrapeSummaryEl) return;
			scrapeSummaryEl.innerHTML = [
				{
					label: "Title",
					value: data.title ? data.title.substring(0, 40) + (data.title.length > 40 ? "..." : "") : "Not found",
					status: data.title ? "success" : "error"
				},
				{
					label: "Brand",
					value: data.brand || "Not found",
					status: data.brand ? "success" : "warning"
				},
				{
					label: "Price",
					value: data.price || "Not found",
					status: data.price ? "success" : "warning"
				},
				{
					label: "Bullet Points",
					value: `${data.bulletPoints?.length || 0} items`,
					status: data.bulletPoints?.length > 0 ? "success" : "warning"
				},
				{
					label: "Description",
					value: data.description ? `${data.description.length} chars` : "Not found",
					status: data.description ? "success" : "warning"
				},
				{
					label: "Specifications",
					value: `${Object.keys(data.specifications || {}).length} items`,
					status: Object.keys(data.specifications || {}).length > 0 ? "success" : "warning"
				}
			].map((c) => `
		...
		const doScrapePreview = () => {
			...
			if (scrapeJsonEl) scrapeJsonEl.innerHTML = syntaxHighlightJSON(data);
			console.log("═══════════════════════════════════════════════════════");
			if (typeof ExtensionConfig !== "undefined" && ExtensionConfig.FEATURES.DEBUG_MODE) {
		...
		const closeScrapeDrawer = () => {
			if (scrapeDrawer) scrapeDrawer.style.display = "none";
		};
		if (scrapePreviewBtn) {
			scrapePreviewBtn.addEventListener("click", openScrapeDrawer);
			console.log("✅ Scrape Preview button listener added");
		}
		if (scrapeDrawerCloseBtn) scrapeDrawerCloseBtn.addEventListener("click", closeScrapeDrawer);
		if (scrapeDrawerOverlay) scrapeDrawerOverlay.addEventListener("click", closeScrapeDrawer);
		if (scrapeRefreshBtn) scrapeRefreshBtn.addEventListener("click", doScrapePreview);
		if (scrapeCopyJsonBtn) scrapeCopyJsonBtn.addEventListener("click", async () => {
			if (!lastScrapedData) {
		...
		document.querySelectorAll("#snipe-title-list .title-row").forEach((row) => {
			row.addEventListener("click", (e) => {
				if (e.target.closest(".btn-copy-title") || e.target.closest(".title-input")) return;
				selectTitleRow(row);
			});
		});
		...
		document.querySelectorAll(".btn-copy-title").forEach((btn) => {
			btn.addEventListener("click", () => {
				...
				if (input && input.value) UIHelper?.copyToClipboard?.(input.value);
				else window.UIHelper?.showToast?.("No title to copy", "warning");
			});
		});
		const optiListBtn = document.getElementById("opti-list-btn");
		if (optiListBtn) {
				...
				chrome.runtime.sendMessage({ action: "CHECK_AUTH" }, async (authResponse) => {
					...
					if (selectedRow || hasAiTitle) {
						...
						try {
							...
							const thirtyMinutesAgo = Date.now() - 1800 * 1e3;
							const useSelectedTitle = titleStorage.selectedTitleTimestamp && titleStorage.selectedTitleTimestamp >= thirtyMinutesAgo && titleStorage.selectedEbayTitle && titleStorage.selectedEbayTitle.trim() !== "";
							let selectedTitle;
							if (useSelectedTitle) {
		...
		if (priceInput) {
			...
			priceInput.addEventListener("input", () => {
				if (!priceInput._ssCalcFill && typeof window.SSListingDraft !== "undefined") {
					...
					if (!isNaN(fp) && fp > 0) window.SSListingDraft.patchDraft({
						price_source: "manual",
		...
		if (sellItForEl && sellItForEl !== priceInput) sellItForEl.addEventListener("input", () => {
		...
		if (skuInput) {
			...
			skuInput.addEventListener("input", () => {
				if (!skuInput._ssAutoSku && typeof window.SSListingDraft !== "undefined") {
					...
					if (sku) window.SSListingDraft.patchDraft({
						sku,
						sku_source: "manual"
					}).catch(() => {});
				}
	...
	function openCalculator() {
		...
		if (popup) {
			...
			if (amazonPriceInput) {
				...
				} else console.log("⚠️ No fresh Amazon price scraped on open");
			}
			calculatePrice();
	...
	async function getProductDataForExport() {
		...
		const thirtyMinutesAgo = Date.now() - 1800 * 1e3;
		const selectedTitleIsRecent = storageData.selectedTitleTimestamp && storageData.selectedTitleTimestamp >= thirtyMinutesAgo;
		let title = "";
	...
	async function loadSKUSettings() {
		try {
			...
			chrome.storage.local.get([
				...
				let generatedSku = "";
				if (mode === "asin") generatedSku = getASIN() || "NO-ASIN";
				else if (mode === "title") {
					const words = (document.getElementById("productTitle")?.innerText.trim() || "").replace(/[^a-zA-Z0-9 ]/g, "").split(" ");
				...
				} else if (mode === "custom") generatedSku = prefix;
				if (useSuffix) {
	...
	async function generateSKU() {
		try {
			...
			if (selectedTitle && ebayPrice && amazonPrice) try {
				chrome.runtime.sendMessage({
	...
	var injectInlineButtons = () => {
		document.querySelectorAll("div.s-card-container").forEach((card) => {
		...
		if (!!document.getElementById("productTitle") || !!document.getElementById("dp-container") || !!document.querySelector("#buybox, #desktop_buybox")) {
			const buyBox = document.querySelector("#buybox, #desktop_buybox, #rightCol, #centerCol");
			if (buyBox) {
				if (buyBox.parentNode && !document.getElementById("initial-list-button-container")) {
					const wrapper = document.createElement("div");
			...
			} else console.log("[Amazon Injector] Product page detected but buyBox element not found.");
		}
	};
	...
	var initializeApp = async () => {
		if (window.location.hash.includes("sellersuit_auto_list") || window.location.search.includes("sellersuit_auto_list")) {
			...
			const checkReady = () => {
				return !!document.getElementById("productTitle") || !!document.getElementById("dp-container") || !!document.querySelector("#buybox, #desktop_buybox");
			};
	...
	function _applyPricingToProduct(product, extValues) {
		...
		let savedValues = {};
		try {
			savedValues = JSON.parse(localStorage.getItem("calculatorValues") || "{}");
		} catch (_) {}
		if (extValues && typeof extValues === "object" && Object.keys(extValues).length > 0) savedValues = {
			...savedValues,
			...extValues
		};
		const parseVal = (v, def) => {
		...
		const cfg = {
			...
			trackingFee: parseVal(savedValues["tracking-fee"], .2),
			...
			paymentFixedFee: parseVal(savedValues["payment-fixed-fee"], .3)
		};
		const priceOne = (raw) => {
			const r = calculateSellingPrice({
				sourcePrice: cleanFloat(raw),
				...cfg
			});
			return r ? r.finalPrice : .99;
		};
		const baseRaw = cleanFloat(product.price);
		product.raw_supplier_price = baseRaw;
		if (!(product.price_source === "manual" && cleanFloat(product.finalPrice) > 0)) product.finalPrice = priceOne(baseRaw);
		if (Array.isArray(product.variants)) product.variants.forEach((v) => {
	...
	chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
		...
		if (request.action === "PEEK_PRODUCT") {
			try {
				...
				const mainImg = (document.getElementById("landingImage") || document.getElementById("imgBlkFront"))?.getAttribute("data-old-hires") || document.getElementById("landingImage")?.src || "";
				let variantCount = 0;
				try {
					const jqData = window.P && window.P.getNow && window.P.getNow({ "features": ["twister"] })?.data?.colorImages;
					if (jqData) {
						const allAsins = /* @__PURE__ */ new Set();
						Object.values(jqData).forEach((arr) => Array.isArray(arr) && arr.forEach((e) => e.asin && allAsins.add(e.asin)));
						variantCount = allAsins.size;
					}
				} catch (_) {}
				if (variantCount === 0) variantCount = document.querySelectorAll("#variation_color_name li, #variation_size_name li, .swatchAvailable, .swatchSelect").length;
				let selectedLabel = "";
				const selColor = document.querySelector("#variation_color_name .selection, #variation_color_name .a-color-base")?.textContent?.trim();
				const selSize = document.querySelector("#variation_size_name .selection, #variation_size_name .a-color-base")?.textContent?.trim();
		...
		if (request.action === "SCRAPE_SINGLE") {
			(async () => {
				try {
					...
					let _extCalcValues = {};
					try {
						_extCalcValues = (await new Promise((r) => chrome.storage.local.get("calculatorValues", r))).calculatorValues || {};
					} catch (_) {}
					...
					if (!product.finalPrice || product.finalPrice <= 0) console.warn("[SS SCRAPE_SINGLE] finalPrice missing or zero — check calculator values");
					...
					} else await chrome.storage.local.set({ currentProduct: product });
					sendResponse({
						success: true,
						data: product
					});
		...
		if (request.action === "SCRAPE_VARIANTS") {
			(async () => {
				try {
					...
					let _extCalcValues2 = {};
					try {
						_extCalcValues2 = (await new Promise((r) => chrome.storage.local.get("calculatorValues", r))).calculatorValues || {};
					} catch (_) {}
					...
					if (!product.finalPrice || product.finalPrice <= 0) console.warn("[SS SCRAPE_VARIANTS] finalPrice missing or zero — check calculator values");
					if (typeof window.SSListingDraft !== "undefined") {
		...
		if (request.action === "PREPARE_EBAY_LISTING") {
			(async () => {
				try {
					let fullData;
					if (request.options?.skipScrape) fullData = (await chrome.storage.local.get("currentProduct")).currentProduct;
					if (!fullData) {
						const _adapter = window.SSSupplierRegistry?.match(location.href);
						if (_adapter) try {
							const _raw = await _adapter.scrapeVariants(request.options || {
								minQty: 0,
								allowLowQty: true
							});
						...
						else fullData = scrapeFullProductData();
					...
					let _extCalcValues3 = {};
					try {
						_extCalcValues3 = (await new Promise((r) => chrome.storage.local.get("calculatorValues", r))).calculatorValues || {};
					} catch (_) {}
					const extProduct = (await new Promise((r) => chrome.storage.local.get("currentProduct", r))).currentProduct || {};
					...
					if (Array.isArray(fullData.variants) && Array.isArray(extProduct.variants)) fullData.variants.forEach((fv) => {
						const match = extProduct.variants.find((ev) => {
							...
							if (fv.attrs && ev.attrs && Object.keys(fv.attrs).length === Object.keys(ev.attrs).length) return Object.entries(fv.attrs).every(([k, v]) => {
								const vName = v && typeof v === "object" ? v.productName : v;
								const evVal = ev.attrs[k];
								return vName === (evVal && typeof evVal === "object" ? evVal.productName : evVal);
							});
							return false;
						});
	...
	//#endregion
	//#region content_scripts/amazon_fulfiller.js
	(async () => {
...
//# sourceMappingURL=amazon.bundle.js.map