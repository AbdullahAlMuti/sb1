(function() {
  ...
  `;
	...
	//#endregion
	//#region content_scripts/aliexpress_scraper.js
	window.SSAliExpressScraper = (() => {
		...
		function highResImage(url) {
			return absoluteUrl(url).replace(/_(\d+x\d+|[0-9]+x[0-9]+q[0-9]+)\.(jpg|jpeg|png|webp)$/i, ".$2").replace(/_(\d+x\d+)\.(jpg|jpeg|png|webp)\?.*$/i, ".$2");
		}
		...
		function cleanPrice(value) {
			...
			if (typeof value === "object") return cleanPrice(value.value ?? value.amount ?? value.minAmount ?? value.maxAmount ?? value.minActivityAmount ?? value.formatedAmount);
			const match = String(value).replace(/,/g, "").match(/-?\d+(?:\.\d+)?/);
			return match ? parseFloat(match[0]) : null;
		}
		function currencyFrom(value, fallback = "USD") {
			if (!value || typeof value !== "object") return fallback;
			return value.currencyCode || value.currency || value.currencySymbol || value.code || fallback;
		}
		...
		function readBalancedObject(scriptText, marker) {
			...
			for (let i = open; i < scriptText.length; i++) {
				...
				if (ch === "{") depth++;
				if (ch === "}") depth--;
				if (depth === 0) return scriptText.slice(open, i + 1);
			}
			return null;
		}
		...
		function walk(root, visitor, seen = /* @__PURE__ */ new Set()) {
			if (!root || typeof root !== "object" || seen.has(root)) return void 0;
			seen.add(root);
		...
		function findComponent(objects, names) {
			for (const object of objects) {
				const found = walk(object, (node) => {
					for (const name of names) if (node[name] && typeof node[name] === "object") return node[name];
				});
		...
		function findFirstValue(objects, keys) {
			for (const object of objects) {
				const found = walk(object, (node) => {
					for (const key of keys) if (node[key] != null && node[key] !== "") return node[key];
				});
		...
		function extractImages(objects, doc) {
			...
			add(findFirstValue(objects, [
				"image",
				"images",
				"imagePathList"
			]));
			add(meta(doc, ["meta[property=\"og:image\"]", "meta[name=\"twitter:image\"]"]));
			if (doc.querySelectorAll) Array.from(doc.querySelectorAll("img")).forEach((img) => {
		...
		function extractSpecs(objects, doc) {
			const specs = {};
			const source = findComponent(objects, [
				"specsModule",
				"specsComponent",
				"productPropComponent"
			]) || findFirstValue(objects, [
				"productProperties",
				"props",
				"properties"
			]);
			...
			const rows = Array.isArray(source) ? source : source && typeof source === "object" ? source.props || source.productProperties || source.properties || source.specs : null;
			if (Array.isArray(rows)) rows.forEach((item) => addPair(item.attrName || item.name || item.key || item.propertyName, item.attrValue || item.value || item.propertyValue));
			else if (source && typeof source === "object") Object.entries(source).forEach(([key, value]) => {
				if (typeof value === "object") addPair(value.attrName || value.name || key, value.attrValue || value.value || value.propertyValue);
				else addPair(key, value);
			});
			if (doc.querySelectorAll) Array.from(doc.querySelectorAll("tr, li, dl div")).forEach((row) => {
		...
		function skuAmount(sku) {
			...
			return cleanPrice(val.skuAmount) ?? cleanPrice(val.actSkuCalPrice) ?? cleanPrice(val.discountPrice) ?? cleanPrice(val.price) ?? cleanPrice(sku.price);
		}
		...
		function extractVariants(objects, product) {
			const skuComponent = findComponent(objects, [
				"skuComponent",
				"skuModule",
				"skuInfo"
			]);
			...
			props.forEach((prop) => {
				...
				(prop.skuPropertyValues || prop.values || prop.propertyValues || []).forEach((value) => {
					const key = `${propId}:${text(value.propertyValueId || value.valueId || value.vid || value.id)}`;
					lookup.set(key, {
						label,
		...
		function extractProductDocument(doc = document, url = window.location.href) {
			...
			const productInfo = findComponent(objects, [
				"productInfoComponent",
				"productInfo",
				"itemModule"
			]) || {};
			const priceComponent = findComponent(objects, [
				"priceComponent",
				"priceModule",
				"priceInfo"
			]) || {};
			const sellerComponent = findComponent(objects, [
				"storeInfoComponent",
				"sellerComponent",
				"storeModule"
			]) || {};
			const shippingComponent = findComponent(objects, [
				"shippingModule",
				"shippingComponent",
				"deliveryComponent"
			]) || {};
			const title = text(productInfo.subject || productInfo.title || productInfo.name) || text(ldProduct && ldProduct.name) || meta(doc, ["meta[property=\"og:title\"]", "meta[name=\"twitter:title\"]"]) || domText(doc, ["h1", "[data-pl=\"product-title\"]"]);
			if (!title) throw new Error("AliExpress product title not found");
			const price = cleanPrice(priceComponent.discountPrice) ?? cleanPrice(priceComponent.origPrice) ?? cleanPrice(priceComponent.formatedPrice) ?? cleanPrice(ldProduct?.offers?.price) ?? cleanPrice(meta(doc, ["meta[property=\"product:price:amount\"]"]));
			const currency = currencyFrom(priceComponent.discountPrice || priceComponent.origPrice || priceComponent, "") || text(ldProduct?.offers?.priceCurrency) || meta(doc, ["meta[property=\"product:price:currency\"]"]) || "USD";
			const images = extractImages(objects, doc);
			const specs = extractSpecs(objects, doc);
			const description = text(productInfo.description || findFirstValue(objects, ["description"])) || text(ldProduct && ldProduct.description) || meta(doc, ["meta[name=\"description\"]"]);
			const seller = {
			...
			const shipping = {
				text: text(shippingComponent.shippingText || shippingComponent.deliveryProviderName || shippingComponent.freightCommitDay || shippingComponent.displayAmount),
			...
			if (product.variantExtractionStatus === "selected-only") product.variantExtractionWarnings = ["Only the currently selected AliExpress variant was available."];
			return product;
		}
	...
	//#endregion
	//#region content_scripts/aliexpress_injector.js
	(function() {
		"use strict";
		...
		async function storedCalculatorValues() {
			try {
				return (await new Promise((resolve) => chrome.storage.local.get("calculatorValues", resolve))).calculatorValues || {};
			} catch (_) {
				return {};
			}
		}
		function applyPricing(product, calculatorValues) {
			...
			const parseVal = (key, fallback) => {
				return cleanFloat(saved[key]) || fallback;
			};
			const cfg = {
				taxPercent: parseVal("tax-percent", 9),
				trackingFee: parseVal("tracking-fee", .2),
				...
				paymentFixedFee: parseVal("payment-fixed-fee", .3)
			};
			const baseRaw = cleanFloat(product.price || product.raw_supplier_price);
			product.raw_supplier_price = baseRaw;
			if (baseRaw > 0 && !(product.price_source === "manual" && cleanFloat(product.finalPrice) > 0)) product.finalPrice = window.SSPricingEngine.calculatePrice(baseRaw, cfg);
			if (Array.isArray(product.variants)) product.variants.forEach((variant) => {
				const raw = cleanFloat(variant.price || variant.raw_supplier_price) || baseRaw;
				variant.raw_supplier_price = raw;
				if (raw > 0 && !(cleanFloat(variant.ebayPrice) > 0)) variant.finalPrice = window.SSPricingEngine.calculatePrice(raw, cfg);
			});
			return product;
		}
		...
		async function injectUI({ fromSidebar = false, sidebarImages = [] } = {}) {
			...
			const panelContent = new DOMParser().parseFromString(html, "text/html").getElementById("snipe-root-wrapper");
		...
		function openSidePanel(autoScan = false) {
			try {
				chrome.runtime.sendMessage({ action: "OPEN_SIDE_PANEL" }, (response) => {
					...
					if (err || response?.ok === false) console.warn("[AliExpress Injector] Could not open side panel:", err?.message || response?.error);
		...
		function createListButton() {
			...
			wrapper.style.cssText = "position:fixed;right:18px;bottom:18px;z-index:2147483646;display:block;margin:0;padding:0;border-radius:8px;box-shadow:0 8px 24px rgba(15,23,42,0.22);";
    ...
    `;
			btn.style.cssText = "display:inline-flex;align-items:center;justify-content:center;padding:10px 14px;border-radius:8px;background:#0654ba;color:#fff;text-decoration:none;cursor:pointer;border:0;font-family:-apple-system,BlinkMacSystemFont,\"Segoe UI\",Roboto,sans-serif;font-weight:600;font-size:14px;line-height:1;transition:opacity 0.2s,transform 0.2s;";
		...
		function initializeApp() {
			...
			if (location.hash.includes("sellersuit_auto_list") || location.search.includes("sellersuit_auto_list")) {
		...
		chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
			...
			if (request.action === "SCRAPE_SINGLE" || request.action === "SCRAPE_VARIANTS") {
				(async () => {
					try {
						sendResponse({
							success: true,
							data: await scan(request.action === "SCRAPE_SINGLE" ? "single" : "all", request.options || {})
						});
					} catch (err) {
						sendResponse({
							success: false,
			...
			if (request.action === "PREPARE_EBAY_LISTING") {
				(async () => {
					try {
						let fullData;
						if (request.options && request.options.skipScrape) fullData = (await chrome.storage.local.get("currentProduct")).currentProduct;
			...
			if (request.action === "EXTEND_PANEL") {
				(async () => {
					try {
						...
						await injectUI({
							fromSidebar: true,
							sidebarImages: Array.isArray(stored.currentProduct?.images) ? stored.currentProduct.images : []
						});
		...
		else document.addEventListener("DOMContentLoaded", () => observer.observe(document.body, {
			childList: true,
			subtree: true
		}));
	})();
...
//# sourceMappingURL=aliexpress.bundle.js.map