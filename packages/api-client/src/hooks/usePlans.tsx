import { useState, useEffect, useCallback } from 'react';
import { supabase } from '@repo/api-client/supabase/client';

export interface PlanFeature {
  id: string;
  plan_id: string;
  group_name: string;
  title: string;
  description: string | null;
  display_value: string | null;
  included: boolean;
  tooltip: string | null;
  is_highlighted: boolean;
  sort_order: number;
}

export interface Plan {
  id: string;
  name: string;
  display_name: string;
  price_monthly: number;
  price_yearly: number;
  features: string[];
  plan_features: PlanFeature[];
  stripe_price_id_monthly: string | null;
  stripe_price_id_yearly: string | null;
  stripe_price_id_one_time: string | null;
  feature_flags: Record<string, unknown>;
  max_listings: number;
  max_auto_orders: number;
  credits_per_month: number;
  is_active: boolean;
  is_trial: boolean;
  is_popular: boolean;
  is_recommended: boolean;
  is_public: boolean;
  trial_duration_days: number;
  trial_requires_card: boolean;
  auto_orders_enabled: boolean;
  seo_enabled: boolean;
  max_seo_titles: number;
  max_seo_descriptions: number;
  order_reset_frequency: string;
  slug: string | null;
  short_description: string | null;
  best_for: string | null;
  badge_text: string | null;
  cta_text: string | null;
  stripe_product_id: string | null;
  archived_at: string | null;
}

export function usePlans() {
  const [plans, setPlans] = useState<Plan[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const fetchPlans = useCallback(async () => {
    try {
      const { data, error: fetchError } = await supabase
        .from('plans')
        .select(`
          *,
          plan_features (
            id, plan_id, group_name, title, description,
            display_value, included, tooltip, is_highlighted, sort_order
          )
        `)
        .eq('is_active', true)
        .eq('is_public', true)
        .is('archived_at', null)
        .order('sort_order', { ascending: true });

      if (fetchError) {
        throw fetchError;
      }

      const formattedPlans: Plan[] = (data || []).map(plan => {
        const rawFeatures = (plan as any).plan_features as PlanFeature[] ?? [];
        const sortedFeatures = [...rawFeatures].sort((a, b) => a.sort_order - b.sort_order);

        return {
          id: plan.id,
          name: plan.name,
          display_name: plan.display_name || plan.name,
          price_monthly: plan.price_monthly || 0,
          price_yearly: plan.price_yearly || 0,
          features: Array.isArray(plan.features)
            ? (plan.features as unknown as string[])
            : [],
          plan_features: sortedFeatures,
          stripe_price_id_monthly: plan.stripe_price_id_monthly,
          stripe_price_id_yearly: plan.stripe_price_id_yearly,
          stripe_price_id_one_time: (plan as any).stripe_price_id_one_time ?? null,
          feature_flags: ((plan as any).feature_flags as Record<string, unknown>) ?? {},
          max_listings: (plan as any).max_listings ?? 0,
          max_auto_orders: (plan as any).max_auto_orders ?? 0,
          credits_per_month: (plan as any).credits_per_month ?? 0,
          is_active: plan.is_active ?? true,
          is_trial: (plan as any).is_trial ?? false,
          is_popular: (plan as any).is_popular ?? false,
          is_recommended: (plan as any).is_recommended ?? false,
          is_public: (plan as any).is_public ?? true,
          trial_duration_days: (plan as any).trial_duration_days ?? 14,
          trial_requires_card: (plan as any).trial_requires_card ?? true,
          auto_orders_enabled: (plan as any).auto_orders_enabled ?? true,
          seo_enabled: (plan as any).seo_enabled ?? true,
          max_seo_titles: (plan as any).max_seo_titles ?? 0,
          max_seo_descriptions: (plan as any).max_seo_descriptions ?? 0,
          order_reset_frequency: (plan as any).order_reset_frequency ?? 'monthly',
          slug: (plan as any).slug ?? null,
          short_description: (plan as any).short_description ?? null,
          best_for: (plan as any).best_for ?? null,
          badge_text: (plan as any).badge_text ?? null,
          cta_text: (plan as any).cta_text ?? null,
          stripe_product_id: (plan as any).stripe_product_id ?? null,
          archived_at: (plan as any).archived_at ?? null,
        };
      });

      setPlans(formattedPlans);
      setError(null);
    } catch (err: unknown) {
      if (err instanceof Error) {
        setError(err.message);
      } else {
        setError('Failed to load plans');
      }
    } finally {
      setIsLoading(false);
    }
  }, []);

  // Initial fetch
  useEffect(() => {
    fetchPlans();
  }, [fetchPlans]);

  // Realtime subscription for immediate updates when plans change
  useEffect(() => {
    const channel = supabase
      .channel('plans-realtime')
      .on(
        'postgres_changes',
        {
          event: '*',
          schema: 'public',
          table: 'plans'
        },
        (payload) => {
          if (import.meta.env.DEV) console.log('Plans changed:', payload.eventType);
          // Refetch plans on any change (INSERT, UPDATE, DELETE)
          fetchPlans();
        }
      )
      .subscribe();

    return () => {
      supabase.removeChannel(channel);
    };
  }, [fetchPlans]);

  const getPlanByName = (name: string) => plans.find(p => p.name === name);
  const getPlanById = (id: string) => plans.find(p => p.id === id);

  return {
    plans,
    isLoading,
    error,
    getPlanByName,
    getPlanById,
    refetch: fetchPlans,
  };
}
