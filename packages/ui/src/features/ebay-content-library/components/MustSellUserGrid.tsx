...
import { Search, TrendingUp, HelpCircle, Package, Plus, ExternalLink, Flame, Crown, Award, Medal } from 'lucide-react';
...
import { MustSellItem } from '@repo/marketplace-core/features/ebay-content-library/types/content-library.types';
import { useMustSellItems } from '@repo/marketplace-core/features/ebay-content-library/hooks/useMustSellItems';
...
export function MustSellUserGrid() {
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCountry, setSelectedCountry] = useState<string>('all');
  const [selectedCategory, setSelectedCategory] = useState<string>('all');
  const [currentPage, setCurrentPage] = useState(1);

  const { items, isLoading } = useMustSellItems({
    mode: 'user',
    searchQuery,
    selectedCountry,
  });
  ...
  const handleAddToEbay = (item: MustSellItem) => {
    const listingData = {
      title: item.title,
      price: item.price,
      image_url: item.image_url,
      category: item.category,
      source: 'must-sell-items',
